...
checkK8sRunning() {
  if docker ps > /dev/null 2>&1; then
    ...
    echo -e "$Red Docker is not running, please start Docker for desktop! $ColorOff"
  fi

  if kubectl get pods > /dev/null 2>&1; then
    echo -e "$Green Kubernetes is running and we are able to connect! $ColorOff"
  else
    echo -e "$Red Could not connect to kubernetes, please check Docker for Desktop settings https://docs.docker.com/desktop/kubernetes/#enable-kubernetes $ColorOff"
  fi
}
